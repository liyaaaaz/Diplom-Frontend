...
const filterOptions = [
  ...
  { value: "plywood", label: "Только фанера" },
  { value: "timber", label: "Только брус" },
  { value: "fiberboard", label: "Только ДВП" },
];

export const PlywoodPage = () => {
        ...
        .filter((product) => {
          switch (filterOption) {
            case "price":
              return true;
            case "plywood":
              return product.subcategory.name === "Фанера";
            case "timber":
              return product.subcategory.name === "Брус";
            case "fiberboard":
                return product.subcategory.name === "ДВП";
            default:
              return true;
          }
        })
      ...
      <Box className="grid grid-cols-3 max-xl:grid-cols-2 max-lg:grid-cols-1 gap-4">
        {products ? (
          filteredProducts.length > 0 ? (
            filteredProducts
            .filter((product) => product.subcategory.category === "PlywoodTimberFiberboard")
            .map((product, i) => {
              {
                return (
                  <div key={i}>
