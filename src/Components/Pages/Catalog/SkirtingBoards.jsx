...
const filterOptions = [
  ...
  { value: "floor", label: "Только напольные" },
  { value: "ceiling", label: "Только потолочные" },
];

export const SkirtingBoardsPage = () => {
        ...
        .filter((product) => {
          switch (filterOption) {
            case "price":
              return true;
            case "floor":
              return product.subcategory.name === "Напольные";
            case "ceiling":
              return product.subcategory.name === "Потолочные";
            default:
              return true;
          }
        })
      ...
      <Box className="grid grid-cols-3 max-xl:grid-cols-2 max-lg:grid-cols-1 gap-4">
        {products ? (
          filteredProducts.length > 0 ? (
            filteredProducts
            .filter((product) => product.subcategory.category === "SkirtingBoards")
            .map((product, i) => {
              {
                return (
                  <div key={i}>
