...
const filterOptions = [
  ...
  { value: "liquidnails", label: "Только жидкие гвозди" },
  { value: "wallpaperglue", label: "Только клей для обоев" },
  { value: "sealants", label: "Только герметики" },
  { value: "silicones", label: "Только силиконы" },
  { value: "PVA", label: "Только ПВА" },
];

export const AdhesivesPage = () => {
        ...
        .filter((product) => {
          switch (filterOption) {
            case "price":
              return true;
            case "liquidnails":
              return product.subcategory.name === "Жидкие гвозди";
            case "wallpaperglue":
              return product.subcategory.name === "Клей для обоев";
            case "sealants":
                return product.subcategory.name === "Герметики";
            case "silicones":
                return product.subcategory.name === "Силиконы";
            case "PVA":
                return product.subcategory.name === "ПВА";
            default:
              return true;
          }
        })
      ...
      <Box className="grid grid-cols-3 max-xl:grid-cols-2 max-lg:grid-cols-1 gap-4">
        {products ? (
          filteredProducts.length > 0 ? (
            filteredProducts
            .filter((product) => product.subcategory.category === "AdhesivesSealantsSilicones")
            .map((product, i) => {
              {
                return (
                  <div key={i}>
