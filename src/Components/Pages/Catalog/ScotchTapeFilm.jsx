...
const filterOptions = [
  ...
  { value: "scotchtape", label: "Только скотч" },
  { value: "filmwithtape", label: "Только пленка со скотчем" },
  { value: "polyethylenefilm", label: "Только полиэтиленовая пленка" },
];

export const ScotchPage = () => {
        ...
        .filter((product) => {
          switch (filterOption) {
            case "price":
              return true;
            case "scotchtape":
              return product.subcategory.name === "Скотч";
            case "filmwithtape":
              return product.subcategory.name === "Пленка со скотчем";
            case "polyethylenefilm":
                return product.subcategory.name === "Полиэтиленовая пленка";
            default:
              return true;
          }
        })
      ...
      <Box className="grid grid-cols-3 max-xl:grid-cols-2 max-lg:grid-cols-1 gap-4">
        {products ? (
          filteredProducts.length > 0 ? (
            filteredProducts
            .filter((product) => product.subcategory.category === "ScotchTapeFilm")
            .map((product, i) => {
              {
                return (
                  <div key={i}>
