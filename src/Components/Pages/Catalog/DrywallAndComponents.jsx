...
const filterOptions = [
  ...
  { value: "GKL", label: "Только ГКЛ" },
  { value: "profilesforGKL", label: "Только профили для ГКЛ" },
  { value: "accessoriesforprofiles", label: "Только комплектующие для профилей" },
  { value: "paintingcorners", label: "Только малярные углы" },
];
        ...
        .filter((product) => {
          switch (filterOption) {
            case "price":
              return true;
            case "GKL":
              return product.subcategory.name === "ГКЛ";
            case "profilesforGKL":
              return product.subcategory.name === "Профили для ГКЛ";
            case "accessoriesforprofiles":
                return product.subcategory.name === "Комплектующие для профилей";
            case "paintingcorners":
                return product.subcategory.name === "Малярные углы";
            default:
              return true;
          }
        })
      ...
      <Box className="grid grid-cols-3 max-xl:grid-cols-2 max-lg:grid-cols-1 gap-4">
        {products ? (
          filteredProducts.length > 0 ? (
            filteredProducts
            .filter((product) => product.subcategory.category === "DrywallAndComponents")
            .map((product, i) => {
              {
                return (
                  <div key={i}>
